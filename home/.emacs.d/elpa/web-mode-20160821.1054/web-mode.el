...
;; Copyright 2011-2016 FranÃ§ois-Xavier Bois

;; Version: 14.0.21
;; Package-Version: 20160821.1054
;; Author: FranÃ§ois-Xavier Bois <fxbois AT Google Mail Service>
;; Maintainer: FranÃ§ois-Xavier Bois
...
;; NOTE: il est important d'identifier des caractÃ¨res en fin de ligne
...
;; todo : parsing plus compliquÃ©: {$obj->values[3]->name}
...
;; todo : passer de rÃ¨gle en rÃ¨gle et mettre un \n Ã  la fin
...
(defun web-mode-previous-line (pos limit)
  (save-excursion
    (let (beg end line (continue t))
      ...
      (if (<= (point) limit)
          ;;todo : affiner (le + 3 n est pas gÃ©nÃ©rique cf. <?php <% <%- etc.)
     ...
     (controls
      ...
      (while continue
        (cond
         ...
         (t
          ;;TODO : est il nÃ©cessaire de faire un reverse sur controls si on doit matcher backward
...
(defun web-mode-dom-apostrophes-replace ()
  "Replace char(') with char(â€™) in the html contents of the buffer."
  (interactive)
  (save-excursion
...
(defun web-mode-dom-entities-replace ()
  "Replace html entities (e.g. &eacute; &#233; or &#x00E9; become Ã©)"
  (interactive)
  (save-excursion
...
(defun web-mode-trace (msg)
  (let (sub)
    ...
    (message "%18s: time elapsed = %Ss %9SÂµs" msg (nth 1 sub) (nth 2 sub))
    ))
